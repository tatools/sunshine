...
class Junit5StatusTest {
    ...
    void codePassed() {
        MatcherAssert.assertThat(
                new Junit5Status(new Summary(1, 0, 0, 0)).code(), Matchers.is((short) 0));
    }

    @Test
    void codeFailed() {
        MatcherAssert.assertThat(
                new Junit5Status(new Summary(1, 0, 0, 1)).code(), Matchers.is((short) 1));
    }

    @Test
    void runCount() {
        MatcherAssert.assertThat(
                new Junit5Status(new Summary(5, 4, 3, 1)).runCount(), Matchers.is(5));
    }

    @Test
    void failureCount() {
        MatcherAssert.assertThat(
                new Junit5Status(new Summary(5, 4, 3, 1)).failureCount(), Matchers.is(4));
    }

    @Test
    void ignoreCount() {
        MatcherAssert.assertThat(
                new Junit5Status(new Summary(5, 4, 3, 1)).ignoreCount(), Matchers.is(3));
    }
    ...
    private final class Summary implements TestExecutionSummary {
        ...
        public Summary(
                long totalTests, long failedCount, long skippedCount, long totalFailedCount) {
